...
import { AppBar, Toolbar, ToggleButton } from '@mui/material';
...
const Sidebar: React.FC<SidebarProps> = ({ onDateChange, sidebarToggle }) => {
  ...
  return (
    ...
    <Toolbar className='flex-column' sx={{ display: 'flex', justifyContent: 'space-between' }}>
        ...
        sx={{'& .MuiSvgIcon-root': isDarkMode ? {fill: 'var(--light-color)'} : {},
                '& .MuiInputBase-root': isDarkMode ? {color:'var(--light-color)'} : {},
                '& .MuiFormLabel-root': isDarkMode ? {color: 'var(--light-color)'}: {},
                '& .MuiInputBase-input': isDarkMode ? {backgroundColor: 'var(--dark-blue-color)', color:'white'}:{backgroundColor: 'var(--lighter-color)'},
                marginRight: 'inherit'}}
