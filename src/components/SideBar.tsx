...
const Sidebar: React.FC<SidebarProps> = React.memo(({ onDateChange, sidebarToggle }) => {
  ...
  // Custom Popper component for DatePicker
  const CustomPopper = (props: any) => {
    return (
      <Popper
        {...props}
        placement="bottom-start"
        className={`custom-popper ${isDarkMode ? 'dark-mode' : ''}`}
      />
    );
  };
  ...
  const handleDateChange = (date: Date | null) => {
    ...
    onDateChange(date); // Trigger callback to pass the date to Stepper
  };
        ...
        <Toolbar className='flex-column' sx={{ display: 'flex', justifyContent: 'space-between' }}>
          ...
          <div className='navigation-tools flex-column aStart'>
            ...
            <DatePicker
              ...
              slots={{
                popper: CustomPopper, // Use custom popper
              }}
              sx={{display: "none"}}
            />
          </div>
